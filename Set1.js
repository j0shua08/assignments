...
function savings(grossPay, taxRate, expenses) {
   ...
   let moneyremaining = grossPay - expenses - tax
   return Math.floor(moneyremaining)
...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    const materialsconsumed = (numJobs * jobConsumption) 
    const waste = (totalMaterial - materialsconsumed)
    return `${waste}${materialUnits}`;
...
function interest(principal, rate, periods) {
    let quant = rate * periods
   let si = principal + (principal * quant)
